...
class _EditTaskPageState extends State<EditTaskPage> {
  ...
  void initState() {
    super.initState();
    editedTask = widget.arg.task;
    titleController.text = editedTask.title;
    descriptionController.text = editedTask.description;
            ...
            children: [
              ...
              Row(
                  ...
                  children: [
                    ...
                    Text(formatDueTime(editedTask.duetime),
                        style: const TextStyle(
                          fontSize: 14,
                          fontWeight: FontWeight.w500,
                          height: 18 / 14,
                ...
                children: [
                  SizedBox(
                    ...
                    child: Stack(
                      ...
                      children: [
                        for (var i = 0;
                            i < min(editedTask.teamMembers.length, 3);
                            i++)
                          Positioned(
                            left: (i * 22),
                            top: 0,
                            child: CircleAvatar(
                              ...
                              child: Container(
                                ...
                                child: Image.asset(
                                  editedTask.teamMembers[i].avatarUrl,
                                ),
                              ),
                            ),
                          ),
                  ...
                  if (editedTask.teamMembers.length > 3)
                    Flexible(
                      child: Padding(
                        padding: const EdgeInsets.only(bottom: 6),
                        child: Text(
                          "+${editedTask.teamMembers.length - 3}",
                          style: const TextStyle(
                            color: Colors.black,
                          ),
                        ),
              ...
              Column(
                children: [
                  for (var i = 0; i < editedTask.stages.length; i++)
                    TaskStageInput(
                      onRemove: () {
                        setState(() {
                          editedTask.stages.removeWhere((element) =>
                              element.uid == editedTask.stages[i].uid);
                        });
                      },
                      onChecked: (value) {
                        setState(() {
                      ...
                      onTap: () {
                        setState(() {
                          final TaskStage newStage = TaskStage(
                              taskUid: editedTask.uid,
                              isDone: false,
                              stageName: "",
                              uid: const Uuid().v4());
                          editedTask.stages.add(newStage);
                        });
                      },
                    ),
                  )
